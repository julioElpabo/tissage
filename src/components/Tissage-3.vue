        ...
        >
          ...
          <g>
            ...
            <path
              ...
              :d="seg.d"
              :fill="seg.fill"
              :filter="seg.filter ? seg.filter : null"
            />
            <circle cx="100" cy="100" r="30" fill="red" filter="url(#ombre-dessous)" stroke="black" stroke-width="1" />
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              <div class="d-flex align-items-center mb-2" @mouseleave="addColorToModal">
                <input type="color" v-model="modalColor" class="form-control form-control-color me-2" />
              </div>
              <div class="d-flex flex-wrap mb-3" style="gap:4px;">
                <div
                  v-for="(c, i) in modalCouleurs"
                  :key="'mcolor-' + i"
                  :style="{ backgroundColor: c, width: '24px', height: '24px', border: '1px solid #000' }"
                  @click="modalCouleurs.splice(i, 1)"
                  title="Supprimer cette couleur"
                ></div>
              </div>
              <label class="form-label">Épaisseur</label>
              <input type="number" v-model.number="modalEpaisseur" step="0.1" class="form-control" />
            </div>
...
export default {
  ...
  return {
    ...
    confirmAddFil() {
      const fil = {
        couleurs: [...this.modalCouleurs],
        epaisseur: this.modalEpaisseur
      };
      if (this.modalType === 'horizontal') {
        if (this.editingIndex !== null) this.filsHorizontal.splice(this.editingIndex, 1, fil);
        else this.filsHorizontal.push(fil);
      } else {
        if (this.editingIndex !== null) this.filsVertical.splice(this.editingIndex, 1, fil);
        else this.filsVertical.push(fil);
      }
      this.closeModal();
      this.updatePreview();
    },
